 ...
 * Version 1.1 Update 2026/1/25
 * 此脚本可供任意地图作者使用,使用时还请标明出处QAQ
 */
//服务器配置
...
const boss_template = Instance.FindEntityByName("boss_template");//存放boss相关实体的template
const boss_position = Instance.FindEntityByName("boss_spawn_point").GetAbsOrigin();//boss生成位置,需要将任意类实体命名,并放置在想要的位置
...
2、info_target(命名为boss_spawn_point,若想修改为自定义名称,请修改上述参数中的boss_position = Instance.FindEntityByName("XXXXXXXX").GetAbsOrigin(),将括号内的名称替换即可)
3、func_phys_box(创建物理碰撞实体,命名为boss_physbox,与配置中boss_phys_targetname相同,若想自定义名称,像1、2步操作即可)将该实体存入之前创建的point_template中,并为该实体添加ondamaged 对脚本实体输入runscriptinput,参数boss_hp_subract(用于boss减血)
...
7、创建point_script实体(r任意命名),,并在属性内的script选择该脚本路径
8、创建任意触发类实体,对point_script这个实体输入runscriptinput参数填script_init,触发后即可生成boss
9、若对上述说明存有遗忘,请参考示例地图boss_template.vmap中的实体放置
*/
...
class BossMain {
    ...
    boss_init() {
        ...
        if (enemy.length > 0) {
            ...
            Instance.Msg("say 未找到符合条件玩家,自动结束追踪");
            this.isActive = false;
        }
    }
    target_change() {
...
class QueueMain {
    constructor() {
        ...
        this.thinkIndex = -1;//控制循环是否进行,-1为终止
    }
    startthink() {
        Instance.SetThink(() => {
            if (this.thinkIndex == -1 || !deafult_boss.phy?.IsValid()) return;
